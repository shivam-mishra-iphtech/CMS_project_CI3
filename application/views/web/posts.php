                        ...
                        <div class="comment d-flex gap-4 my-2">
                            ...
                            <div>
                                <div class="d-flex gap-3 align-items-center mb-2">
                                    <h4 class="h6 mb-0">
                                        <?= !empty($comment->userName) ? $comment->userName : 'Anonymous' ?></h4>
                                    <small
                                        class="text-muted"><?= date('M d, Y', strtotime($comment->created_at)) ?></small>
                                </div>
                                <p>
                                    <?php
                            $content = strip_tags(htmlspecialchars($comment->user_coment));
                            $short = strlen($content) > 100 ? substr($content, 0, 150) : $content;
                            $full = $content;
                            $isLong = strlen($content) > 100;
                        ?>
                                    <span class="short-comment"><?= $short ?><?= $isLong ? '...' : '' ?></span>
                                    <?php if ($isLong): ?>
                                    <span class="full-comment d-none"><?= $full ?></span>
            ...
            <div class="sticky-top" style="top: 20px;">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-4" >
                                <img style="border-radius:5px ;" src="<?php echo base_url('public/postImages/'.$related_post->thumbnail); ?>"
                                    class="card-img-top" alt="Related Post"
                                    onerror="this.onerror=null; this.src='<?php echo base_url('public/banners/logo2.jpg'); ?>'">

                            </div>
                            <div class="col-8">
                                <a href="<?= site_url('WebController/view_post_by_id/'.$related_post->id); ?>"
